...
const Edit = (props) => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (navigator.onLine) {
        ...
        if (existingData) {
          // Assign new data to replace the existing data
          const updatedData = {
            id,
            ...medicalFormData,
            fileUrl: `${minioEndpoint}/${bucketName}/${fileName}`,
          ...
          await store.delete(existingData._id);
    
          console.log('Update complete.');
          // Pass updatedData to the function
        } else {
 ...
 const handleOnline = async () => {
  ...
  window.addEventListener('online', handleOnline);
    ...
    async function syncDataWithMongoDB(updatedData) {
      if (navigator.onLine) {
        // Send the updated data to MongoDB
        try {
          const response = await fetch('http://localhost:5000/medicalForm', {
            method: 'PUT',
            headers: {
          ...
          if (response.ok) {
            ...
            console.error('Failed to sync data with MongoDB.');
            // Handle errors, retry logic, or other appropriate actions
          }
        } catch (error) {
          console.error('Error during data sync with MongoDB:', error);
          // Handle errors, retry logic, or other appropriate actions
        }
      } else {
        console.warn('Application is offline. Data will be synced when online.');
      }
    }
  };
...
//           const offlineData = await store.get(id);

//           if (offlineData) {
//             const { disease, ...otherData } = offlineData;

//             // Initialize the state with the data received from IndexedDB
//             setMedicalFormData({
//               ...otherData,
//               disease: disease || [], // Ensure disease is an array even if it's null
//             });
//           } else {
//             console.log(`Data for ID ${id} not found in IndexedDB`);
//           }
//         }
//       } catch (error) {
//         console.error('Error fetching data:', error);
...
//       // Online: Upload file and update data in MongoDB
//       if (file) {
//         fileName = `medical_form_${Date.now()}_${file.name}`;
//         await minioUploader(file, fileName);
...
//       // Modify the stored data with the new values
//       const updatedData = { ...medicalFormData, fileUrl: `${minioEndpoint}/${bucketName}/${fileName}` };
//       await store.put(updatedData);  // No need to provide 'id' as a parameter

//       console.log('Data updated in IndexedDB:', updatedData);
//       setMedicalFormData(updatedData);
//     }
//   } catch (error) {
//     console.error('Error:', error);
//   }
